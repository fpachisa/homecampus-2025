...
import random
from Database import HCResetPassword
...
class BaseHandler(RequestHandler, Jinja2Mixin):
    ...
    def redirect_path(self, default='/'):
        if '_continue' in self.session:
            ...
            url = self.request.args.get('continue', default)  
...
class PracticeLoginHandler(BaseHandler):
    ...
    def post(self, **kwargs):
        ...
        if source == 'Facebook' or source =='Google+':
            user = self.auth.get_user_entity(username=username)
            '''if existing user ..we are redirecting them to the page they came from after signing in..
                else if the user is created for the first we redirect them to MyProfile page so that they can add child account'''
            response = Response("")
            if not user:
                '''User is not registered with Home Campus..we are creating the user'''
                email = username
                if source == 'Facebook':
                    logging.info("User logging in with Facebook")
                    auth_id_parent = 'FB|%s' % email
                elif source == 'Google+':
                    logging.info("User logging in with Google")
                    auth_id_parent = 'G+|%s' % email
                    
                parent_password = ''
                parent_first_name = self.request.form.get('first_name')
                parent_last_name = self.request.form.get('last_name')
                user = self.auth.create_user(email, auth_id_parent, password=parent_password,first_name=parent_first_name,
                                             last_name=parent_last_name,email=email,IsParent=True
                                             )                
                response = Response(source)
            auth_id = user.auth_id
            while not self.auth.user:
                self.auth.login_with_auth_id(auth_id, True)
            if self.auth.user:               
                response.status_code = 200
                return response
        else:
            self.auth.login_with_form(username, password)
            if self.auth.user:
                return self.redirect(redirect_url)
...
class LoginHandler(BaseHandler):
    ...
    def post(self, **kwargs):
        ...
        #Popup Login Support
        #added on 12-07-2016 by Riyaz
        async = True if self.request.args.get('async') is not None else False
        
        if self.auth.user:
            # if it's an async request than simply redirect the user to redirect_url
            # This condition should not arise in normal use cases
            if async:
                return Response(json_encode({'success': True, 'continue_url': redirect_url}))
            else:
                return self.redirect(redirect_url)
        ...
        if source == 'Facebook' or source =='Google+':
            ...
            # 13-07-2016 -- Riyaz
            #Removing textual-response as all social-login requests are now async and response will be in JSON!
            #response = Response("")
            ...
            if not user:
                ...
                email = username
                if source == 'Facebook':
                    logging.info("User signing up with Facebook")
                    auth_id_parent = 'FB|%s' % email
                elif source == 'Google+':
                    logging.info("User signing up with Google")
                    auth_id_parent = 'G+|%s' % email
                ...
                #response = Response(source)
                # First-time login (social registration), so redirect to /MyProfile
            ...
            if self.auth.user and async:               
                #User is now logged in
                #Async request, returning JSON-response
                return Response(json_encode({'success': True, 'continue_url': redirect_url})) #Create a new response object with json output                
            else:
                # non-Async request are forbidden
                response = Response("Forbidden Access")
            ...
            if self.auth.user:
                if async:        
                    #Create a new response object with json output
                    return Response(json_encode({'success': True, 'continue_url': redirect_url}))
                else:
                    return self.redirect(redirect_url)
        ...
        if async:
            ...
            return Response(json_encode(ctx)) #Create a new response object with json output
        else:
            self.session['login_error'] = 'Either username or password is incorrect. Please try again.'
            return self.get(**kwargs)
...
class RegisterHandler(BaseHandler):
    ...
    def post(self, **kwargs):
        ...
        # Popup Login Support added on 14-07-2016 by Riyaz
        async = True if self.request.args.get('async') is not None else False
        ...
        if parent_user:
            registration_message = registration_message + "This email is already registered."
        ...
        try:
            if self.AlreadyRegistered:
                ...
                if async:
                    return Response(json_encode({'success': True, 'continue_url': "/MyProfile"}))
                else:
                    return self.redirect("/MyProfile")
        ...
        if async:
            return Response(json_encode({'success': False, 'error': registration_message}))
        else:
...
class PasswordHandler(BaseHandler):
    def get(self, **kwargs):
        ...
        if self.session.get('password_error'):
            ...
            kwargs.update({'password_error':password_error})

        return self.render_response('ForgotPasswordPage.html', **kwargs)
    ...
    def post(self,**kwargs):
        ...
        if ParentChildChoice == "parent":
            username = unicode(self.request.form.get('parent_username'))
            email = username
            IsParent = True
        elif ParentChildChoice == "child":
            username = unicode(self.request.form.get('child_username'))
        ...
        user = self.auth.get_user_entity(username=username) 
        if user:
            '''If signed up using G+ of FB ...then can't reset password'''
            if user.auth_id[:2]=="G+":
                password_error.append("You have signed up using Google. Click Google sign in button to login.")
                self.session['password_error'] = password_error
                return self.get(**kwargs)
            if user.auth_id[:2]=="FB":
                password_error.append("You have signed up using Facebook. Click Facebook sign in button to login.")
                self.session['password_error'] = password_error
                return self.get(**kwargs)
                            
            if unicode(user.email).lower() != email.lower():
        ...
        ResetKey = ''.join(random.choice(string.ascii_uppercase + string.digits + string.ascii_lowercase) for _x in range(16))
        expiry_date = datetime.datetime.now() + datetime.timedelta(hours=24)
        
        HCResetPWEntry = HCResetPassword.HCResetPassword(username=username,email=email,reset_key=ResetKey,IsChild=IsChild,
                                                         IsParent=IsParent,expiry_date=expiry_date)
        HCResetPWEntry.put()
        
        reset_link = "http://my.homecampus.com.sg/ResetPassword?key="+str(ResetKey)+"&username="+username
        message = mail.EmailMessage()
        message.to = email
        message.sender = "Home Campus <admin@homecampus.com.sg>"
        message.subject = "Home Campus - Reset Password"
        SendMessage = '<html><head></head><body><!--CTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dt--><meta content="text/html; charset=utf-8" /><meta content="en-us" />'
        SendMessage = SendMessage + '<table description="lightestBgcolor" name="tid" cellpadding="0" cellspacing="0" border="0"><tbody><tr><td style="padding: 0px 20px 20px 20px;"><div style="padding: 10px; margin: 0px;"><table align="left" width="560" cellpadding="0" cellspacing="0" style="font-family:Verdana;"><tbody><tr><td style="padding: 0px 20px; background: url(http://my.homecampus.com.sg/images/NewsLetter/HomeCampus_newsletter_header.png) no-repeat center;"><table description="mediumBorderBottomColor" name="tid" width="560" height="129" cellpadding="10" cellspacing="0" border="0"><tbody><tr><td style="text-align:left; width="300">&nbsp;</td><td ><br /><br /><h3 valign="baseline" align="right" style="font-family: Verdana, Helvetica, sans-serif; font-size: 14px; padding: 2px 0px; margin: 1px;"><b><font color="#D92118">Learn.</font> <font color="#2A91E1">Practice.</font> <font color="#F6B937">Progress.</font></b></h3><h3 valign="baseline" align="right" style="font-family: Verdana, Helvetica, sans-serif; font-weight: bold; color: #006666; font-size: 13px; padding: 2px 0px; margin: 1px;"><a href="http://my.homecampus.com.sg" style="text-decoration:none;"><font color="#006666">www.homecampus.com.sg</font></a></h3></td></tr></tbody></table></td></tr><tr><td bgcolor="#ffffff" style="padding: 20px 20px 20px 20px;"><table width="560" cellpadding="0" cellspacing="0" border="0" ><tbody><tr><td valign="top" style="text-align:left;padding-bottom: 0px; padding-left: 0px; padding-right: 20px; padding-top: 0px;" colspan=2><p style="font-size: 12px; line-height: 1.7em; font-family:Verdana; color: #333333;">'
        SendMessage = SendMessage + 'Dear '+user.first_name+' '+user.last_name+','
        SendMessage = SendMessage + '<br /><br />Please click <a href='+reset_link+'>here</a> to reset password for your Home Campus account.<br><br>'
        SendMessage = SendMessage + 'If you are unable to click, paste the link below in your browser and proceed.<br><br>'
        SendMessage = SendMessage + reset_link
        logging.info(reset_link)
        message.html = SendMessage
        message.Send()
        
        kwargs.update({'ResetPWFormSubmitted':"Y"})
        return self.get(**kwargs)

class ResetPasswordHandler(BaseHandler):
    def get(self, **kwargs):                   
       
        ResetKey = str(self.request.args.get('key'))
        username = str(self.request.args.get('username'))

        ValidLink = False
        ExpiredLink = True
        
        '''Checking link validity'''
        ResetLinkData = HCResetPassword.HCResetPassword.gql("where reset_key = '"+ResetKey+"' and username = '"+username+"'").fetch(1)
        for r in ResetLinkData:
            ValidLink = True
            if r.expiry_date >= datetime.datetime.now() and not r.password_changed:
                ExpiredLink = False
        
        kwargs.update({'ValidLink':ValidLink,'ExpiredLink':ExpiredLink,'username':username})
        
        return self.render_response('ResetPasswordPage.html', **kwargs)
    
    def post(self,**kwargs):
        redirect_url = self.redirect_path()
        username = self.request.form.get('username')
        password = self.request.form.get('new_password')
        email = username
        user = self.auth.get_user_entity(username=username) 
        user.set_password(password)
        user.put()
        self.auth.login_with_auth_id(user.auth_id, True)
        ResetLinkData = HCResetPassword.HCResetPassword.gql("where username = '"+username+"' order by create_date desc").fetch(1)
        for r in ResetLinkData:
